...
import { MAX_NICKNAME_LENGTH, validateNickname } from "./validator";

describe("validateNickname", () => {
  it("유효한 닉네임이면 빈 문자열을 반환해야 한다.", () => {
    expect(validateNickname("오밀오밀")).toBe("");
    expect(validateNickname("hello123")).toBe("");
    expect(validateNickname("닉네임 ok")).toBe("");
  });
  ...
  it("여러 특수문자가 포함된 경우 모든 특수문자를 에러 메시지에 포함해야 한다.", () => {
    const result = validateNickname("test!@");
    expect(result).toContain("!");
    expect(result).toContain("@");
  });
  ...
  it("최대 길이 검사를 특수문자보다 먼저 수행해야 한다.", () => {
    ...
    const result = validateNickname(tooLongWithSpecial);
    expect(result).toBe(`닉네임은 최대 ${MAX_NICKNAME_LENGTH}자까지 입력할 수 있습니다.`);
  });
});
